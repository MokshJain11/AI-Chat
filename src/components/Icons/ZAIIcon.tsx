type ZAIIconProps = {
    className?: string
}

export default function ZAIIcon({ className }: ZAIIconProps) {
    return (
        <svg
            className={className}
            viewBox="1.17 1.17 27.66 27.65"
            xmlns="http://www.w3.org/2000/svg"
        >
            {/* Background */}
            <path
                fill="currentColor"
                d="M24.51 28.51H5.49c-2.21 0-4-1.79-4-4V5.49c0-2.21 1.79-4 4-4h19.03c2.21 0 4 1.79 4 4v19.03c0 2.21-1.79 4-4 4Z"
            />

            {/* Z mark */}
            <path
                fill="white"
                d="M15.47 7.1l-1.3 1.85c-.2.29-.54.47-.9.47h-7.1V7.09c0 .01 9.3.01 9.3.01Z"
            />

            <polygon
                fill="white"
                points="24.3,7.1 13.14,22.91 5.7,22.91 16.86,7.1"
            />

            <path
                fill="white"
                d="M14.53 22.91l1.31-1.86c.2-.29.54-.47.9-.47h7.09v2.33h-9.3Z"
            />
        </svg>
    )
}